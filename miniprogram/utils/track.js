...
// ============ 🔵 攒批上报：高频事件，满 20 / 定时 / 切后台合并发 ============
function track(event, params) {
  queue.push(buildEvent(event, params));
  if (queue.length > MAX_PENDING) queue = queue.slice(-MAX_PENDING);
  if (queue.length >= BATCH_SIZE) {
    syncToStorage();   // flush 前先落缓存
    flush();
  }
}
...
function flush() {
  ...
  flushing = true;
  const batch = queue.slice(0, BATCH_SIZE); // 取前 20 条（原数组不动）
...
// 定时 flush：前台每 5 秒清一次，切后台时队列几乎为空
function startTimer() {
  setInterval(() => { if (queue.length) { syncToStorage(); flush(); } }, 5000);
}
...
function pageEnter(route, extra) {
  ...
  if (lastPage && lastPage._route) {
    track('page_leave', { page: lastPage._route, duration_ms: now - (lastPage._enterTs || now) });
  }
  lastPage = { _route: route, _enterTs: now };
  track('page_view', Object.assign({ page: route }, extra));
}
